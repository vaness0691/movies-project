...
const PercentageBar = ({ rating }: { rating: number }) => {
    ...
    let bgColor: string = 'bg-lightGreen';
    ...
    return (
        <div className="flex w-full items-center gap-2">
            <div className="flex relative w-full h-2.5 bg-gray-300 rounded-lg">
                <div className={`${bgColor} absolute h-full left-0 rounded-lg text-xs`}
                    style={{ width: `${ratingTPercentage}%`}}>
                </div>
            </div>
            <div className="flex text-xs text-darkBlue">{ratingTPercentage > 0 ? `${ratingTPercentage}%` : 'NR'}</div>
        </div>
    );
};
